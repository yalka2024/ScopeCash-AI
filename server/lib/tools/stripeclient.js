/**
 * HTTP integration adapter: StripeClient
 * Checkout session creation, subscription management, idempotent webhook handling, invoice retrieval, and billing state sync.
 *
 ...
 *     STRIPECLIENT_BASE_URL=https://api.example.com/...   (STRIPECLIENT_API_KEY=...)
 *     INTEGRATION_STRIPECLIENT_MODE=live
 * Until a base URL is set, live mode REFUSES (501) rather than fake a result.
 */
const NAME = "StripeClient";
const ENV_KEY = 'INTEGRATION_STRIPECLIENT_MODE';
const BASE_URL_KEY = 'STRIPECLIENT_BASE_URL';
const API_KEY_KEY = 'STRIPECLIENT_API_KEY';
...
module.exports = {
  name: NAME,
  description: "Checkout session creation, subscription management, idempotent webhook handling, invoice retrieval, and billing state sync.",
  inputs: ["plan_id", "customer_email", "idempotency_key"],
  outputs: ["checkout_url", "subscription_id", "invoice_id", "webhook_event"],
  envKey: ENV_KEY,
