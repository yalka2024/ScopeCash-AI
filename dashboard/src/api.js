...
/**
 * EU AI Act classifier — runs the rules engine on a record and persists the
 * verdict (overallRisk, riskScore, reportJson). Returns { record, verdict }.
 */
export async function classifyRecord(id, input) {
  return apiJson(`/projects/${id}/classify`, {
    method: 'POST',
...
/**
 * EU AI Act enums — sectors, decision impacts, sensitive data, scopes, roles.
 * Cached client-side via the standard browser Cache-Control header.
 */
export async function getEuAiActEnums() {
  return apiJson('/eu-ai-act/enums');
}

/**
 * Annex IV technical documentation — JSON form (machine-readable).
 */
export async function getAnnexIvJson(id) {
  return apiJson(`/projects/${id}/annex-iv.json`);
}

/**
 * Annex IV technical documentation — PDF download.
 * Triggers a file download in the browser; returns the blob size.
 */
export async function downloadAnnexIvPdf(id) {
  const res = await apiFetch(`/projects/${id}/annex-iv.pdf`);
  if (!res.ok) {
  ...
  a.download = m ? m[1] : `annex-iv-${id}.pdf`;
...
export async function getConformity(id) {
  const r = await apiFetch(`/projects/${id}/conformity`);
  if (!r.ok) throw new Error(`HTTP ${r.status}`);
  return r.json();
}
export async function startConformity(id, route = 'internal_control_annex_vi') {
  const r = await apiFetch(`/projects/${id}/conformity/start`, {
...
export async function updateConformityItem(id, itemId, patch) {
  const r = await apiFetch(`/projects/${id}/conformity/item`, {
...
export async function attestConformity(id, signatureName, signatureRole) {
  const r = await apiFetch(`/projects/${id}/conformity/attest`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ signatureName, signatureRole }),
  });
  if (!r.ok) {
    let body = null; try { body = await r.json(); } catch {}
    throw new Error((body && body.error) || `HTTP ${r.status}`);
...
// ─── Model Evaluations (Article 15) ──────────────────────────────
export async function listEvalSuites() {
  const r = await apiFetch(`/projects/system/eval-suites`);
  if (!r.ok) throw new Error(`HTTP ${r.status}`);
  return r.json();
}
export async function listEvaluations(id) {
  const r = await apiFetch(`/projects/${id}/evaluations`);
  if (!r.ok) throw new Error(`HTTP ${r.status}`);
  return r.json();
}
export async function getEvaluation(id, runId) {
  const r = await apiFetch(`/projects/${id}/evaluations/${runId}`);
  if (!r.ok) throw new Error(`HTTP ${r.status}`);
  return r.json();
}
export async function runEvaluation(id, body) {
  const r = await apiFetch(`/projects/${id}/evaluations/run`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
  if (!r.ok) {
    let bod = null; try { bod = await r.json(); } catch {}
    throw new Error((bod && bod.error) || `HTTP ${r.status}`);
  }
  return r.json();
}
